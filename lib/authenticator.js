...
exports.Authenticator = class {
    ...
    authenticate(req, res, next) {
        ...
        try {
            ...
            if (this.storageOptions.enabled){
                req.session = new Session(decoded.uid, decoded.customClaims, this.storageOptions);
            } else {
    ...
    generateToken(uid, customClaims={}) {
        if (this.storageOptions.enabled){
            let session = new Session(uid, customClaims, this.storageOptions);
        } else {
            let session = new Session(uid, customClaims);
        }
        let token = jwt.sign({
            uid: uid,
